...
# Merge tag used inside the exported email body, resolved by the sending tool
# from the `first_line` column (Smartlead/Instantly: double curly braces, name
# must match the mapped column). The drafted body generated by emailer holds
# the real opener as plain text; at export time we swap that opener back out
# for this tag so one opening is not emitted twice (once literal, once variable).
FIRST_LINE_TAG = "{{first_line}}"


def _with_first_line_tag(body: str, first_line: str) -> str:
    """Replaces the opener inside the drafted body with the sending tool's
    merge tag. `first_line` is the exact extracted opener (see _first_line_from);
    swapping it back out makes the exported body reference the variable rather
    than duplicating the text. If the opener cannot be located the body is
    returned unchanged."""
    if not body or not first_line:
        return body
    idx = body.find(first_line)
    if idx == -1:
        return body
    return body[:idx] + FIRST_LINE_TAG + body[idx + len(first_line):]
...
def _instantly_row(lead: dict) -> dict:
    """One Instantly/Smartlead row built from a lead dict (no DB). Filtering
    (approved status etc.) is the caller's job — the campaign Ship path hands
    this already-decided leads so the same mapping is reused without a query."""
    ...
    return {
        ...
        # The body ships with the {{first_line}} tag where the opener sits;
        # the real text lives in the `first_line` column for the tool to
        # substitute (see _with_first_line_tag).
        "email_body": _with_first_line_tag(lead.get("email_body") or "", first_line),
...
def _iter_instantly_rows(conn, session_id=None, approved_only=True):
    ...
    The lead id is carried on the dict (not written to the CSV) so callers
    can record which leads were exported in export_history."""
    import db as dbmod
    ...
    for lead in leads:
        ...
        if not (lead.get("email") or "").strip():
            continue
        yield _instantly_row(lead)


def instantly_csv_for_leads(leads: list) -> str:
    """CSV text for a pre-selected list of leads — the Ship path's per-batch
    export. Same columns as instantly_csv_string."""
    buf = io.StringIO()
    writer = csv.DictWriter(buf, fieldnames=INSTANTLY_FIELDS)
    writer.writeheader()
    for lead in leads:
        row = _instantly_row(lead)
        writer.writerow({k: row[k] for k in INSTANTLY_FIELDS})
    return buf.getvalue()
...
def instantly_csv_string(conn, session_id=None, approved_only=True) -> str:
    ...
    for row in _iter_instantly_rows(conn, session_id=session_id, approved_only=approved_only):
        # Rows carry an internal "id" key for the download route's
        # DNC/export-history recording — never written to the CSV.
        writer.writerow({k: row[k] for k in INSTANTLY_FIELDS})
    return buf.getvalue()
