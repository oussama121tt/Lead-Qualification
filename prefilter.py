...
# Stage-0 patterns are compiled from the profile [icp] marker lists (see
# profiles/<name>.toml), which also carry the reasoning for what is
# deliberately NOT rejected. This module only applies them.


def evaluate_person(person: dict, *, max_headcount: int | None = None,
                    min_headcount: int | None = None, profile=None) -> dict:
    ...
    A None headcount bound falls back to the profile [icp] band.
    """
    p = profile or load_profile()
    if max_headcount is None:
        max_headcount = p.icp.max_headcount
    if min_headcount is None:
        min_headcount = p.icp.min_headcount
    ...
    if company and p.agency_company_re and p.agency_company_re.search(company):
        return {"decision": "reject", "reason": f"agency/consultancy company name: '{company}'"}
    if company and p.dev_shop_re and p.dev_shop_re.match(company):
        return {"decision": "reject", "reason": f"dev-shop naming pattern (X Software/Technologies): '{company}'"}
    if title and p.agency_title_re and p.agency_title_re.search(title):
    ...
    if title and p.non_decision_title_re and p.non_decision_title_re.search(title) and not (
            p.founder_title_re and p.founder_title_re.search(title)):
        return {"decision": "reject", "reason": f"non-decision-maker title: '{title}'"}

    # 4. Strong founder persona → keep.
    if title and p.founder_title_re and p.founder_title_re.search(title):
...
def evaluate_unclear_with_llm(person: dict, cost_cb=None, profile=None) -> dict:
    ...
    The system prompt is the profile [icp] llm_prefilter_system."""
    ...
    try:
        ...
        t0 = _time.monotonic()
        data, meta = provider.generate_json(prompt, system=p.icp.llm_prefilter_system,
...
def prefilter_people(people: list[dict], *, max_headcount: int | None = None,
                     min_headcount: int | None = None, use_llm: bool = False,
                     cost_cb=None, profile=None) -> dict:
    ...
    for p in people:
        ...
        if v["decision"] == "unclear":
            if use_llm:
                v = evaluate_unclear_with_llm(p, cost_cb=cost_cb, profile=profile)
                unclear_llm += 1
            else:
