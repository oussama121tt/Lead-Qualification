...
def save_scoring_profile(conn, session_id: int, profile_dict: dict, compiler_model: str) -> None:
    """Saves the compiled scoring profile for a session."""
    import datetime
    conn.execute(
        """
        INSERT INTO scoring_profiles (session_id, target_segments, disqualifying_segments,
                                      raw_checkboxes, raw_custom_text, compiled_at, compiler_model)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT (session_id) DO UPDATE SET
            target_segments = EXCLUDED.target_segments,
            disqualifying_segments = EXCLUDED.disqualifying_segments,
            raw_checkboxes = EXCLUDED.raw_checkboxes,
            raw_custom_text = EXCLUDED.raw_custom_text,
            compiled_at = EXCLUDED.compiled_at,
            compiler_model = EXCLUDED.compiler_model
        """,
        (
            session_id,
            json.dumps(profile_dict.get("target_segments", []), ensure_ascii=False),
            json.dumps(profile_dict.get("disqualifying_segments", []), ensure_ascii=False),
            json.dumps(profile_dict.get("raw_checkboxes", []), ensure_ascii=False),
            profile_dict.get("raw_custom_text", ""),
...
def get_scoring_profile(conn, session_id: int) -> dict | None:
    """Returns the compiled scoring profile for a session, or None if none exists."""
    row = conn.execute(
        "SELECT target_segments, disqualifying_segments, raw_checkboxes, raw_custom_text, compiled_at, compiler_model "
        "FROM scoring_profiles WHERE session_id = ?",
        (session_id,)
    ).fetchone()
    if not row:
        return None
    return {
        "target_segments": json.loads(row[0]) if row[0] else [],
        "disqualifying_segments": json.loads(row[1]) if row[1] else [],
        "raw_checkboxes": json.loads(row[2]) if row[2] else [],
        "raw_custom_text": row[3] or "",
        "compiled_at": row[4],
        "compiler_model": row[5],
    }
...
def init_db(conn) -> None:
    ...
    # scoring_profiles (one compiled persona per session)
    try:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS scoring_profiles (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                session_id INTEGER NOT NULL REFERENCES analysis_sessions(id) ON DELETE CASCADE,
                target_segments TEXT NOT NULL,        -- JSON: [{"key","description","offer"}, ...]
                disqualifying_segments TEXT NOT NULL, -- JSON: [{"key","description"}, ...]
                raw_checkboxes TEXT,                  -- JSON des cases cochées, pour audit/re-compilation
                raw_custom_text TEXT,                 -- texte libre original de l'utilisateur, pour audit
                compiled_at TEXT NOT NULL,
                compiler_model TEXT                   -- quel modèle a produit cette compilation, pour audit
            )
        """)
        conn.commit()
    except Exception:
