...
def upload_and_review():
    ...
    flash(
        f"Batch {batch_id} importe : {ingest_summary['inserted']} lignes ajoutees, "
        f"{ingest_summary['skipped_no_website']} ignorees. Dedup: email {dedup_summary['exact_email']}, "
...
def results_view(session_id: int):
    ...
    with open_db() as conn:
        ...
        for lead in scores_data:
            ...
            # Les leads SKIPPED (non coches a l'import) vont dans non_validees
            if status == "SKIPPED":
                non_validees.append(lead)
                continue

            # Les erreurs de scoring (API, pas de contenu) vont dans en_attente
